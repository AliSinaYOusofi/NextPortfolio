...
const SkillsCard: NextPage<Props> = (props) => {
    ...
    return (
        ...
        <div className=" p-4 bg-black/40 rounded-lg mt-3 w-[49%]
        relative transition-all duration-300 skills">
            <div className="flex justify-between items-center">
                {/* <props.icon /> */}
                <h2 className="text-[seashell] textxl font-semibold">{props.header}</h2>
                <p className="text-4xl text-gray-500 object-cover rounded-full border-2 border-indigo-500">{props.icon}</p>
            </div>
            <div>
                <p className="mt-2 text-gray-300">{props.text}</p>
            </div>
